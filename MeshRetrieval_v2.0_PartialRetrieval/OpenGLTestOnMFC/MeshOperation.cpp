...
candidateModel candidate_index_array[DATASIZE]; 
...
void RetrieveMesh(void)
{
	...
	for(int file_id=1;file_id<=DATASIZE;file_id++)
	{
		...
		candidate_index_array[file_id-1].id = file_id;
	...
	for(int file_id=1;file_id<=DATASIZE;file_id++)
	{
		...
		candidate_index_array[file_id-1].descriptors_diff = final_diff[file_id-1];
	}

	//sort candidate id according to descriptors' difference (from low to high)
	getDescriptorsSorted(candidate_index_array,0,DATASIZE-1);

	//PartialWeight function goes here
	//allocate partial weights
	for(int idx_p = 0; idx_p < DATASIZE; idx_p++){
		candidate_index_array[idx_p].partial_weight = double(idx_p);
	}

	//candidates get sorted by id(from 1 to 60)
	getIDSorted(candidate_index_array,0,DATASIZE-1);

	double * previous_partial_weights;
	previous_partial_weights = new double [DATASIZE]();

	//add previous weights
	string PartialWeights_filename = "./PartialWeight/PartialWeights.txt";
	ifstream PartialWeights_file(PartialWeights_filename);
	for(int idx_p = 0; idx_p < DATASIZE; idx_p++){
		PartialWeights_file >> previous_partial_weights[idx_p];
		candidate_index_array[idx_p].partial_weight += previous_partial_weights[idx_p];
	}
	PartialWeights_file.close();

	//write current weights to file
	// open file
	ofstream myfile;
	myfile.open(PartialWeights_filename);
	//write file header
	if (myfile.is_open())
	{
		//write current weights
		for(unsigned int idx_p=0;idx_p<DATASIZE;idx_p++)
		{
			myfile<<candidate_index_array[idx_p].partial_weight<< " ";
		}
		myfile.close();
	}

	//sort current partial weights
	getPartialWeightsSorted(candidate_index_array,0,DATASIZE-1);
	...
	delete [] diffDH;
	delete [] final_diff;
	delete [] previous_partial_weights;
...
{
	int candidate_id = candidate_index_array[index-1].id;
