...
int compareDescriptors(const void * a,const void * b){
	 return ((candidateModel*)a)->descriptors_diff - ((candidateModel*)b)->descriptors_diff;
};
...
void getDescriptorsSorted(candidateModel id_array[], int left_id, int right_id)
{
	...
	{
		if(id_array[left_id].descriptors_diff>id_array[i].descriptors_diff)
		{
			flag++;
	...
	getDescriptorsSorted(id_array,left_id,flag-1);
	getDescriptorsSorted(id_array,flag+1,right_id);
}
...
void getPartialWeightsSorted(candidateModel id_array[], int left_id, int right_id)
{
	...
	for(int i = left_id+1; i<=right_id; i++)
	{
		if(id_array[left_id].partial_weight>id_array[i].partial_weight)
		{
			flag++;
			swap(id_array[flag],id_array[i]);
		}
	...
	//swap(array[flag],array[left_id]);
	swap(id_array[flag],id_array[left_id]);
	getPartialWeightsSorted(id_array,left_id,flag-1);
	getPartialWeightsSorted(id_array,flag+1,right_id);
}
...
void getIDSorted(candidateModel id_array[], int left_id, int right_id)
{
	...
	for(int i = left_id+1; i<=right_id; i++)
	{
		if(id_array[left_id].id>id_array[i].id)
		{
			flag++;
			swap(id_array[flag],id_array[i]);
		}
	...
	//swap(array[flag],array[left_id]);
	swap(id_array[flag],id_array[left_id]);
	getIDSorted(id_array,left_id,flag-1);
	getIDSorted(id_array,flag+1,right_id);
}
